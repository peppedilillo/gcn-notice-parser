...
from gcn_parser.fermi.fermi_lat_pos_diag import FermiLATPosDiag
from gcn_parser.fermi.fermi_lat_pos_diag import parse_fermi_lat_pos_diag
...
def test_parse_fermi_lat_pos_diag_datetimes_are_timezone_aware():
    for p in filter(lambda s: s.suffix == ".xml", (FIXTURES / "fermi_lat_pos_diag").iterdir()):
        result = parse_fermi_lat_pos_diag(p.read_bytes())
