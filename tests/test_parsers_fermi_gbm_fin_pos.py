...
from gcn_parser.fermi.fermi_gbm_fin_pos import FermiGBMFinPos
from gcn_parser.fermi.fermi_gbm_fin_pos import parse_fermi_gbm_fin_pos
...
def test_parse_fermi_gbm_fin_pos_datetimes_are_timezone_aware():
    for p in filter(lambda s: s.suffix == ".xml", (FIXTURES / "fermi_gbm_fin_pos").iterdir()):
        result = parse_fermi_gbm_fin_pos(p.read_bytes())
